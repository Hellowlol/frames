...
from frames.hashing import create_imghash, ImageHash, video_frame_by_frame
from bw_plex.misc import to_time
from frames.db import session_scope, Reference_Frame
...
def visulize_intro_from_hashes(videofile, hashes, pause=0.01, end=600):
    ...
    # Inline since it not need for normal stuff.
...
def ref_play(videofile, pause=0.02, end=600, key=None):
    ...
    # Inline since it not needed for normal usage.
    import matplotlib.pyplot as plt
    ...
    def match_event(event):
        ...
        with session_scope() as se:
            rf = Reference_Frame(hash=str(h), type=val, tvdbid=KEY)
            LOG.debug('Added %s %s from %s in %s', str(h), val, POS, videofile)
            se.add(rf)
    ...
    while cap.isOpened():
        if GOGO:
            ...
            if ret:
                ...
                # Resize the frame using cv2 as mathplotlib is slow if not.
